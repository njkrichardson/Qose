...
/**
 * \brief Indicate whether a COSE algorithm is LSM or not.
 *
 * \param[in] cose_algorithm_id    The algorithm ID to check.
 *
 * \returns This returns \c true if the algorithm is LSM and \c false if not.
 *
 * This is a convenience function to check whether a given
 * integer COSE algorithm ID uses the LSM signing algorithm
 * or not.
 *
 ...
 */
static bool
t_cose_algorithm_is_lsm(int32_t cose_algorithm_id);
...
t_cose_algorithm_is_lsm(int32_t cose_algorithm_id)
{
    ...
    static const int32_t lsm_list[] = {
        COSE_ALGORITHM_LSM_20_8,
        COSE_ALGORITHM_LSM_10_8,
        0
    }; /* 0 is a reserved COSE alg ID ans will never be used */

    return t_cose_check_list(cose_algorithm_id, lsm_list);
}
